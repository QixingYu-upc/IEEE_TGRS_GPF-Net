...
class SLIC(object):
    def __init__(self, HSI, labels, n_segments=1000, compactness=20, max_iter=20, sigma=0, min_size_factor=0.3,max_size_factor=2):
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        #####################################显示超像素图片######################################
        # out = mark_boundaries(img[:,:,[3,1,2]], segments)
        # plt.figure()
        # plt.imshow(out)
        # plt.show()
        #####################################显示超像素图片######################################
        ...
        x = np.reshape(img, [-1, d])  # 对于印第安数据集，x的维度为[21025,15]
        # print(x.shape)
    ...
    def get_A(self, sigma: float):
        ...
        (h, w) = self.segments.shape  # [145,145]
        for i in range(h - 2):
            for j in range(w - 2):
                sub = self.segments[i:i + 2, j:j + 2]  # 周围八邻域
...
class LDA_SLIC(object):
    def __init__(self, data, labels, n_component):  # n_component=15
        self.data = data
