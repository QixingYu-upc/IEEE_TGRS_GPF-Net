...
def generate_png(net,net_input,data_gt,device,total_indices, path,A):
    pred_test1,pred_test2,pred_test3 = [],[],[]
    X = net_input
    X = X.to(device)
    net.eval()
    gt_hsi = data_gt
    ...
    classification_map(y_re1, gt_hsi, 300,path + 'FGPN.png')
    print('------Get classification maps successful-------')

# def Draw_Classification_Map(net, data_gh,A,device,name: str, scale: float = 4.0, dpi: int = 200):
#     data_gh = data_gh.to(device)
#     output = net(data_gh)
#     classification_map=torch.argmax(output, 1).reshape([data_gh.shape[0],data_gh.shape[1]]).cpu()+10
#     print(classification_map.shape[0])
#     print(classification_map.shape[1])
#     fig, ax = plt.subplots()
#     numlabel = np.array(classification_map)
...
#     foo_fig.savefig(name + '.jpg', format='png', transparent=True, dpi=dpi, pad_inches=0)